...
class AttendanceServiceImplTest {
    ...
    @MockBean private AccountForAttendancePort accountForAttendancePort;
    @MockBean private CommuteRecordForAttendancePort commuteRecordForAttendancePort;
    @MockBean private AbsentForAttendancePort absentForAttendancePort;
    ...
    public void markingAbsentSuccess() throws Exception {
        ...
        MarkingAbsentReqDto markingAbsentReqDto =
                MarkingAbsentReqDto.builder()
                        .email("test@email.com")
                        .companyId(1L)
                        .accountId(2L)
                        .build();
        AccountForAttendance accountForAttendance =
                AccountForAttendance.builder().id(3L).roles("EMPLOYER").build();
    ...
    public void recordEnterTimeSuccess() throws Exception {
        ...
        RecordEnterTimeReqDto recordEnterTimeReqDto =
                RecordEnterTimeReqDto.builder().accountId(1L).companyId(2L).tardy(false).build();

        // when
        Mockito.doNothing()
                .when(commuteRecordForAttendancePort)
                .recordEnterTime(any(EnterRecord.class));

        // then
        Assertions.assertDoesNotThrow(
                () -> attendanceService.recordEnterTime(recordEnterTimeReqDto));
    }
    ...
    public void recordExitTimeSuccess() throws Exception {
        ...
        RecordExitTimeReqDto recordExitTimeReqDto =
                RecordExitTimeReqDto.builder().accountId(1L).companyId(2L).build();

        // when
        Mockito.when(
                        commuteRecordForAttendancePort.findEnterRecord(
                                any(RecordExitTimeReqDto.class)))
                .thenReturn(3L);
        Mockito.doNothing()
                .when(commuteRecordForAttendancePort)
    ...
    public void recordExitTimeFailed() throws Exception {
        ...
        Assertions.assertThrows(
                EnterRecordNotFoundException.class,
                () -> attendanceService.recordExitTime(recordExitTimeReqDto));
    }
    ...
    public void getMyAttendanceByDayOfTheWeekSuccess() throws Exception {
        ...
        GetAttendanceByDayOfTheWeekReqDto getAttendanceByDayOfTheWeekReqDto =
                GetAttendanceByDayOfTheWeekReqDto.builder()
                        .accountId(1L)
                        .email("test@email.com")
                        .build();
        Long companyId = 2L;
        ...
        List<GetAttendanceByDayOfTheWeekResDto> getAttendanceByDayOfTheWeekResDtoList =
                setDefaultEmployeeAttendanceList();

        // when
        Mockito.when(accountForAttendancePort.findAccountByEmail(anyString()))
                .thenReturn(accountForAttendance);
        Mockito.when(
                        commuteRecordForAttendancePort.getMyAttendanceByDayOfTheWeek(
                                any(Attendance.class)))
                .thenReturn(getAttendanceByDayOfTheWeekResDtoList);
        List<GetAttendanceByDayOfTheWeekResDto> result =
                attendanceService.getMyAttendanceByDayOfTheWeek(
                        getAttendanceByDayOfTheWeekReqDto, companyId, date);

        // then
        Assertions.assertAll(
                () ->
                        Assertions.assertEquals(
                                getAttendanceByDayOfTheWeekResDtoList.get(0).getAccountId(),
                                result.get(0).getAccountId()),
                () ->
                        Assertions.assertEquals(
                                getAttendanceByDayOfTheWeekResDtoList.get(1).getAccountId(),
    ...
    void getAttendanceByDayOfTheWeekForEmployer() throws Exception {
        // given
        AccountForAttendance accountForAttendance =
                AccountForAttendance.builder().id(1L).roles("EMPLOYER").build();
        GetAttendanceByDayOfTheWeekReqDto getAttendanceByDayOfTheWeekReqDto =
                GetAttendanceByDayOfTheWeekReqDto.builder().email("test@email.com").build();
        Long companyId = 2L;
        LocalDate date = LocalDate.now();
        Attendance attendance =
                Attendance.createAttendance(getAttendanceByDayOfTheWeekReqDto, companyId, date);
        List<GetAttendanceByDayOfTheWeekResDto> attendanceByDayOfTheWeekResDtoList =
                setDefaultAttendanceByDayOfTheWeekResDtoList();

        // when
        Mockito.when(accountForAttendancePort.findAccountByEmail(anyString()))
                .thenReturn(accountForAttendance);
        Mockito.when(
                        commuteRecordForAttendancePort.getAttendancesByDayOfTheWeek(
                                any(Attendance.class)))
                .thenReturn(attendanceByDayOfTheWeekResDtoList);
        List<GetAttendanceByDayOfTheWeekResDto> result =
                attendanceService.getAttendancesByDayOfTheWeek(
                        getAttendanceByDayOfTheWeekReqDto, companyId, date);
        ...
        Assertions.assertAll(
                () ->
                        Assertions.assertEquals(
                                attendanceByDayOfTheWeekResDtoList.get(0).getAccountId(),
                                result.get(0).getAccountId()),
                () ->
                        Assertions.assertEquals(
                                attendanceByDayOfTheWeekResDtoList.get(0).getEnterTime(),
                                result.get(0).getEnterTime()),
                () ->
                        Assertions.assertEquals(
                                attendanceByDayOfTheWeekResDtoList.get(0).getExitTime(),
                                result.get(0).getExitTime()),
                () ->
                        Assertions.assertEquals(
                                attendanceByDayOfTheWeekResDtoList.get(0).getUserName(),
                                result.get(0).getUserName()),
                () ->
                        Assertions.assertEquals(
                                attendanceByDayOfTheWeekResDtoList.get(1).getAccountId(),
                                result.get(1).getAccountId()),
                () ->
                        Assertions.assertEquals(
                                attendanceByDayOfTheWeekResDtoList.get(1).getEnterTime(),
                                result.get(1).getEnterTime()),
                () ->
                        Assertions.assertEquals(
                                attendanceByDayOfTheWeekResDtoList.get(1).getExitTime(),
                                result.get(1).getExitTime()),
                () ->
                        Assertions.assertEquals(
                                attendanceByDayOfTheWeekResDtoList.get(1).getUserName(),
                                result.get(1).getUserName()));
    }

    private AccountForAttendance setDefaultEmployeeAccountForAttendance() {
        return AccountForAttendance.builder().id(1L).roles("EMPLOYEE").build();
    }
    ...
    private List<GetAttendanceByDayOfTheWeekResDto> setDefaultEmployeeAttendanceList() {
        List<GetAttendanceByDayOfTheWeekResDto> getAttendanceByDayOfTheWeekResDtoList =
                new ArrayList<>();
        GetAttendanceByDayOfTheWeekResDto getAttendanceByDayOfTheWeekResDto1 =
                GetAttendanceByDayOfTheWeekResDto.builder()
                        ...
                        .build();
        GetAttendanceByDayOfTheWeekResDto getAttendanceByDayOfTheWeekResDto2 =
                GetAttendanceByDayOfTheWeekResDto.builder()
                        .accountId(1L)
                        .userName("test")
                        .enterTime(LocalDateTime.now())
